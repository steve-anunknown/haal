-- Generated by haal-gen. Do not edit manually.
module Haal.Models.MQTT.VerneMQTwoClientWillRetain
    ( VerneMQTwoClientWillRetainInput (..)
    , VerneMQTwoClientWillRetainOutput (..)
    , verneMQTwoClientWillRetain
    ) where

import qualified Data.Set as Set
import Haal.Automaton.MealyAutomaton (MealyAutomaton, mkMealyAutomaton)

data VerneMQTwoClientWillRetainInput
    = In_ConnectC2
    | In_ConnectC1WithWill
    | In_ConnectC1WithWillRetain
    | In_DeleteRetainedC1
    | In_DeleteRetainedC2
    | In_SubscribeC2
    | In_UnSubScribeC2
    | In_DisconnectTCPC1
    | In_DisconnectC1
    deriving (Show, Eq, Ord, Enum, Bounded)

data VerneMQTwoClientWillRetainOutput
    = Out_C1_ConnectionClosed__c2_ConnAck
    | Out_C1_ConnAck__c2_ConnectionClosed
    | Out_C1_ConnectionClosed__c2_ConnectionClosed
    | Out_Empty__c2_ConnAck
    | Out_C1_PubAck__c2_ConnectionClosed
    | Out_Empty__c2_ConnectionClosed
    | Out_C1_ConnAck__Empty
    | Out_C1_ConnectionClosed__Empty
    | Out_C1_ConnectionClosed__c2_PubAck
    | Out_C1_ConnectionClosed__c2_SubAck
    | Out_C1_ConnectionClosed__c2_UnSubAck
    | Out_C1_PubAck__Empty
    | Out_Empty__c2_PubAck
    | Out_Empty__c2_SubAck
    | Out_Empty__c2_UnSubAck
    | Out_C1_ConnectionClosed__c2_SubAck__Pub_c2_my_topic_bye
    | Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    | Out_Empty__c2_SubAck__Pub_c2_my_topic_bye
    deriving (Show, Eq, Ord, Enum, Bounded)

verneMQTwoClientWillRetain :: MealyAutomaton Int VerneMQTwoClientWillRetainInput VerneMQTwoClientWillRetainOutput
verneMQTwoClientWillRetain = mkMealyAutomaton delta lambda (Set.fromList [0..16]) 0
  where
    delta 0 In_ConnectC2 = 1
    delta 0 In_ConnectC1WithWill = 2
    delta 0 In_ConnectC1WithWillRetain = 3
    delta 0 In_DeleteRetainedC1 = 0
    delta 0 In_DeleteRetainedC2 = 0
    delta 0 In_SubscribeC2 = 0
    delta 0 In_UnSubScribeC2 = 0
    delta 0 In_DisconnectTCPC1 = 0
    delta 0 In_DisconnectC1 = 0
    delta 2 In_ConnectC2 = 4
    delta 2 In_ConnectC1WithWill = 0
    delta 2 In_ConnectC1WithWillRetain = 0
    delta 2 In_DeleteRetainedC1 = 2
    delta 2 In_DeleteRetainedC2 = 2
    delta 2 In_SubscribeC2 = 2
    delta 2 In_UnSubScribeC2 = 2
    delta 2 In_DisconnectTCPC1 = 0
    delta 2 In_DisconnectC1 = 0
    delta 1 In_ConnectC2 = 0
    delta 1 In_ConnectC1WithWill = 4
    delta 1 In_ConnectC1WithWillRetain = 5
    delta 1 In_DeleteRetainedC1 = 1
    delta 1 In_DeleteRetainedC2 = 1
    delta 1 In_SubscribeC2 = 6
    delta 1 In_UnSubScribeC2 = 1
    delta 1 In_DisconnectTCPC1 = 1
    delta 1 In_DisconnectC1 = 1
    delta 4 In_ConnectC2 = 2
    delta 4 In_ConnectC1WithWill = 1
    delta 4 In_ConnectC1WithWillRetain = 1
    delta 4 In_DeleteRetainedC1 = 4
    delta 4 In_DeleteRetainedC2 = 4
    delta 4 In_SubscribeC2 = 7
    delta 4 In_UnSubScribeC2 = 4
    delta 4 In_DisconnectTCPC1 = 1
    delta 4 In_DisconnectC1 = 1
    delta 5 In_ConnectC2 = 3
    delta 5 In_ConnectC1WithWill = 8
    delta 5 In_ConnectC1WithWillRetain = 8
    delta 5 In_DeleteRetainedC1 = 5
    delta 5 In_DeleteRetainedC2 = 5
    delta 5 In_SubscribeC2 = 9
    delta 5 In_UnSubScribeC2 = 5
    delta 5 In_DisconnectTCPC1 = 8
    delta 5 In_DisconnectC1 = 1
    delta 8 In_ConnectC2 = 10
    delta 8 In_ConnectC1WithWill = 11
    delta 8 In_ConnectC1WithWillRetain = 12
    delta 8 In_DeleteRetainedC1 = 8
    delta 8 In_DeleteRetainedC2 = 1
    delta 8 In_SubscribeC2 = 13
    delta 8 In_UnSubScribeC2 = 8
    delta 8 In_DisconnectTCPC1 = 8
    delta 8 In_DisconnectC1 = 8
    delta 9 In_ConnectC2 = 3
    delta 9 In_ConnectC1WithWill = 13
    delta 9 In_ConnectC1WithWillRetain = 13
    delta 9 In_DeleteRetainedC1 = 9
    delta 9 In_DeleteRetainedC2 = 9
    delta 9 In_SubscribeC2 = 9
    delta 9 In_UnSubScribeC2 = 5
    delta 9 In_DisconnectTCPC1 = 13
    delta 9 In_DisconnectC1 = 6
    delta 7 In_ConnectC2 = 2
    delta 7 In_ConnectC1WithWill = 6
    delta 7 In_ConnectC1WithWillRetain = 6
    delta 7 In_DeleteRetainedC1 = 7
    delta 7 In_DeleteRetainedC2 = 7
    delta 7 In_SubscribeC2 = 7
    delta 7 In_UnSubScribeC2 = 4
    delta 7 In_DisconnectTCPC1 = 6
    delta 7 In_DisconnectC1 = 6
    delta 11 In_ConnectC2 = 14
    delta 11 In_ConnectC1WithWill = 8
    delta 11 In_ConnectC1WithWillRetain = 8
    delta 11 In_DeleteRetainedC1 = 4
    delta 11 In_DeleteRetainedC2 = 4
    delta 11 In_SubscribeC2 = 15
    delta 11 In_UnSubScribeC2 = 11
    delta 11 In_DisconnectTCPC1 = 8
    delta 11 In_DisconnectC1 = 8
    delta 6 In_ConnectC2 = 0
    delta 6 In_ConnectC1WithWill = 7
    delta 6 In_ConnectC1WithWillRetain = 9
    delta 6 In_DeleteRetainedC1 = 6
    delta 6 In_DeleteRetainedC2 = 6
    delta 6 In_SubscribeC2 = 6
    delta 6 In_UnSubScribeC2 = 1
    delta 6 In_DisconnectTCPC1 = 6
    delta 6 In_DisconnectC1 = 6
    delta 13 In_ConnectC2 = 10
    delta 13 In_ConnectC1WithWill = 15
    delta 13 In_ConnectC1WithWillRetain = 15
    delta 13 In_DeleteRetainedC1 = 13
    delta 13 In_DeleteRetainedC2 = 6
    delta 13 In_SubscribeC2 = 13
    delta 13 In_UnSubScribeC2 = 8
    delta 13 In_DisconnectTCPC1 = 13
    delta 13 In_DisconnectC1 = 13
    delta 12 In_ConnectC2 = 16
    delta 12 In_ConnectC1WithWill = 8
    delta 12 In_ConnectC1WithWillRetain = 8
    delta 12 In_DeleteRetainedC1 = 5
    delta 12 In_DeleteRetainedC2 = 5
    delta 12 In_SubscribeC2 = 15
    delta 12 In_UnSubScribeC2 = 12
    delta 12 In_DisconnectTCPC1 = 8
    delta 12 In_DisconnectC1 = 8
    delta 3 In_ConnectC2 = 5
    delta 3 In_ConnectC1WithWill = 10
    delta 3 In_ConnectC1WithWillRetain = 10
    delta 3 In_DeleteRetainedC1 = 3
    delta 3 In_DeleteRetainedC2 = 3
    delta 3 In_SubscribeC2 = 3
    delta 3 In_UnSubScribeC2 = 3
    delta 3 In_DisconnectTCPC1 = 10
    delta 3 In_DisconnectC1 = 0
    delta 10 In_ConnectC2 = 8
    delta 10 In_ConnectC1WithWill = 14
    delta 10 In_ConnectC1WithWillRetain = 16
    delta 10 In_DeleteRetainedC1 = 10
    delta 10 In_DeleteRetainedC2 = 10
    delta 10 In_SubscribeC2 = 10
    delta 10 In_UnSubScribeC2 = 10
    delta 10 In_DisconnectTCPC1 = 10
    delta 10 In_DisconnectC1 = 10
    delta 15 In_ConnectC2 = 14
    delta 15 In_ConnectC1WithWill = 13
    delta 15 In_ConnectC1WithWillRetain = 13
    delta 15 In_DeleteRetainedC1 = 7
    delta 15 In_DeleteRetainedC2 = 7
    delta 15 In_SubscribeC2 = 15
    delta 15 In_UnSubScribeC2 = 11
    delta 15 In_DisconnectTCPC1 = 13
    delta 15 In_DisconnectC1 = 13
    delta 14 In_ConnectC2 = 11
    delta 14 In_ConnectC1WithWill = 10
    delta 14 In_ConnectC1WithWillRetain = 10
    delta 14 In_DeleteRetainedC1 = 2
    delta 14 In_DeleteRetainedC2 = 14
    delta 14 In_SubscribeC2 = 14
    delta 14 In_UnSubScribeC2 = 14
    delta 14 In_DisconnectTCPC1 = 10
    delta 14 In_DisconnectC1 = 10
    delta 16 In_ConnectC2 = 12
    delta 16 In_ConnectC1WithWill = 10
    delta 16 In_ConnectC1WithWillRetain = 10
    delta 16 In_DeleteRetainedC1 = 3
    delta 16 In_DeleteRetainedC2 = 16
    delta 16 In_SubscribeC2 = 16
    delta 16 In_UnSubScribeC2 = 16
    delta 16 In_DisconnectTCPC1 = 10
    delta 16 In_DisconnectC1 = 10
    delta _ _ = error "haal-gen: undefined transition"
    lambda 0 In_ConnectC2 = Out_C1_ConnectionClosed__c2_ConnAck
    lambda 0 In_ConnectC1WithWill = Out_C1_ConnAck__c2_ConnectionClosed
    lambda 0 In_ConnectC1WithWillRetain = Out_C1_ConnAck__c2_ConnectionClosed
    lambda 0 In_DeleteRetainedC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 0 In_DeleteRetainedC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 0 In_SubscribeC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 0 In_UnSubScribeC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 0 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 0 In_DisconnectC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 2 In_ConnectC2 = Out_Empty__c2_ConnAck
    lambda 2 In_ConnectC1WithWill = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 2 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 2 In_DeleteRetainedC1 = Out_C1_PubAck__c2_ConnectionClosed
    lambda 2 In_DeleteRetainedC2 = Out_Empty__c2_ConnectionClosed
    lambda 2 In_SubscribeC2 = Out_Empty__c2_ConnectionClosed
    lambda 2 In_UnSubScribeC2 = Out_Empty__c2_ConnectionClosed
    lambda 2 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 2 In_DisconnectC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 1 In_ConnectC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 1 In_ConnectC1WithWill = Out_C1_ConnAck__Empty
    lambda 1 In_ConnectC1WithWillRetain = Out_C1_ConnAck__Empty
    lambda 1 In_DeleteRetainedC1 = Out_C1_ConnectionClosed__Empty
    lambda 1 In_DeleteRetainedC2 = Out_C1_ConnectionClosed__c2_PubAck
    lambda 1 In_SubscribeC2 = Out_C1_ConnectionClosed__c2_SubAck
    lambda 1 In_UnSubScribeC2 = Out_C1_ConnectionClosed__c2_UnSubAck
    lambda 1 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Empty
    lambda 1 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 4 In_ConnectC2 = Out_Empty__c2_ConnectionClosed
    lambda 4 In_ConnectC1WithWill = Out_C1_ConnectionClosed__Empty
    lambda 4 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__Empty
    lambda 4 In_DeleteRetainedC1 = Out_C1_PubAck__Empty
    lambda 4 In_DeleteRetainedC2 = Out_Empty__c2_PubAck
    lambda 4 In_SubscribeC2 = Out_Empty__c2_SubAck
    lambda 4 In_UnSubScribeC2 = Out_Empty__c2_UnSubAck
    lambda 4 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Empty
    lambda 4 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 5 In_ConnectC2 = Out_Empty__c2_ConnectionClosed
    lambda 5 In_ConnectC1WithWill = Out_C1_ConnectionClosed__Empty
    lambda 5 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__Empty
    lambda 5 In_DeleteRetainedC1 = Out_C1_PubAck__Empty
    lambda 5 In_DeleteRetainedC2 = Out_Empty__c2_PubAck
    lambda 5 In_SubscribeC2 = Out_Empty__c2_SubAck
    lambda 5 In_UnSubScribeC2 = Out_Empty__c2_UnSubAck
    lambda 5 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Empty
    lambda 5 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 8 In_ConnectC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 8 In_ConnectC1WithWill = Out_C1_ConnAck__Empty
    lambda 8 In_ConnectC1WithWillRetain = Out_C1_ConnAck__Empty
    lambda 8 In_DeleteRetainedC1 = Out_C1_ConnectionClosed__Empty
    lambda 8 In_DeleteRetainedC2 = Out_C1_ConnectionClosed__c2_PubAck
    lambda 8 In_SubscribeC2 = Out_C1_ConnectionClosed__c2_SubAck__Pub_c2_my_topic_bye
    lambda 8 In_UnSubScribeC2 = Out_C1_ConnectionClosed__c2_UnSubAck
    lambda 8 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Empty
    lambda 8 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 9 In_ConnectC2 = Out_Empty__c2_ConnectionClosed
    lambda 9 In_ConnectC1WithWill = Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    lambda 9 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    lambda 9 In_DeleteRetainedC1 = Out_C1_PubAck__Empty
    lambda 9 In_DeleteRetainedC2 = Out_Empty__c2_PubAck
    lambda 9 In_SubscribeC2 = Out_Empty__c2_SubAck
    lambda 9 In_UnSubScribeC2 = Out_Empty__c2_UnSubAck
    lambda 9 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    lambda 9 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 7 In_ConnectC2 = Out_Empty__c2_ConnectionClosed
    lambda 7 In_ConnectC1WithWill = Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    lambda 7 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    lambda 7 In_DeleteRetainedC1 = Out_C1_PubAck__Empty
    lambda 7 In_DeleteRetainedC2 = Out_Empty__c2_PubAck
    lambda 7 In_SubscribeC2 = Out_Empty__c2_SubAck
    lambda 7 In_UnSubScribeC2 = Out_Empty__c2_UnSubAck
    lambda 7 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    lambda 7 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 11 In_ConnectC2 = Out_Empty__c2_ConnectionClosed
    lambda 11 In_ConnectC1WithWill = Out_C1_ConnectionClosed__Empty
    lambda 11 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__Empty
    lambda 11 In_DeleteRetainedC1 = Out_C1_PubAck__Empty
    lambda 11 In_DeleteRetainedC2 = Out_Empty__c2_PubAck
    lambda 11 In_SubscribeC2 = Out_Empty__c2_SubAck__Pub_c2_my_topic_bye
    lambda 11 In_UnSubScribeC2 = Out_Empty__c2_UnSubAck
    lambda 11 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Empty
    lambda 11 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 6 In_ConnectC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 6 In_ConnectC1WithWill = Out_C1_ConnAck__Empty
    lambda 6 In_ConnectC1WithWillRetain = Out_C1_ConnAck__Empty
    lambda 6 In_DeleteRetainedC1 = Out_C1_ConnectionClosed__Empty
    lambda 6 In_DeleteRetainedC2 = Out_C1_ConnectionClosed__c2_PubAck
    lambda 6 In_SubscribeC2 = Out_C1_ConnectionClosed__c2_SubAck
    lambda 6 In_UnSubScribeC2 = Out_C1_ConnectionClosed__c2_UnSubAck
    lambda 6 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Empty
    lambda 6 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 13 In_ConnectC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 13 In_ConnectC1WithWill = Out_C1_ConnAck__Empty
    lambda 13 In_ConnectC1WithWillRetain = Out_C1_ConnAck__Empty
    lambda 13 In_DeleteRetainedC1 = Out_C1_ConnectionClosed__Empty
    lambda 13 In_DeleteRetainedC2 = Out_C1_ConnectionClosed__c2_PubAck
    lambda 13 In_SubscribeC2 = Out_C1_ConnectionClosed__c2_SubAck__Pub_c2_my_topic_bye
    lambda 13 In_UnSubScribeC2 = Out_C1_ConnectionClosed__c2_UnSubAck
    lambda 13 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Empty
    lambda 13 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 12 In_ConnectC2 = Out_Empty__c2_ConnectionClosed
    lambda 12 In_ConnectC1WithWill = Out_C1_ConnectionClosed__Empty
    lambda 12 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__Empty
    lambda 12 In_DeleteRetainedC1 = Out_C1_PubAck__Empty
    lambda 12 In_DeleteRetainedC2 = Out_Empty__c2_PubAck
    lambda 12 In_SubscribeC2 = Out_Empty__c2_SubAck__Pub_c2_my_topic_bye
    lambda 12 In_UnSubScribeC2 = Out_Empty__c2_UnSubAck
    lambda 12 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Empty
    lambda 12 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 3 In_ConnectC2 = Out_Empty__c2_ConnAck
    lambda 3 In_ConnectC1WithWill = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 3 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 3 In_DeleteRetainedC1 = Out_C1_PubAck__c2_ConnectionClosed
    lambda 3 In_DeleteRetainedC2 = Out_Empty__c2_ConnectionClosed
    lambda 3 In_SubscribeC2 = Out_Empty__c2_ConnectionClosed
    lambda 3 In_UnSubScribeC2 = Out_Empty__c2_ConnectionClosed
    lambda 3 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 3 In_DisconnectC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 10 In_ConnectC2 = Out_C1_ConnectionClosed__c2_ConnAck
    lambda 10 In_ConnectC1WithWill = Out_C1_ConnAck__c2_ConnectionClosed
    lambda 10 In_ConnectC1WithWillRetain = Out_C1_ConnAck__c2_ConnectionClosed
    lambda 10 In_DeleteRetainedC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 10 In_DeleteRetainedC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 10 In_SubscribeC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 10 In_UnSubScribeC2 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 10 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 10 In_DisconnectC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 15 In_ConnectC2 = Out_Empty__c2_ConnectionClosed
    lambda 15 In_ConnectC1WithWill = Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    lambda 15 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    lambda 15 In_DeleteRetainedC1 = Out_C1_PubAck__Empty
    lambda 15 In_DeleteRetainedC2 = Out_Empty__c2_PubAck
    lambda 15 In_SubscribeC2 = Out_Empty__c2_SubAck__Pub_c2_my_topic_bye
    lambda 15 In_UnSubScribeC2 = Out_Empty__c2_UnSubAck
    lambda 15 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__Pub_c2_my_topic_bye
    lambda 15 In_DisconnectC1 = Out_C1_ConnectionClosed__Empty
    lambda 14 In_ConnectC2 = Out_Empty__c2_ConnAck
    lambda 14 In_ConnectC1WithWill = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 14 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 14 In_DeleteRetainedC1 = Out_C1_PubAck__c2_ConnectionClosed
    lambda 14 In_DeleteRetainedC2 = Out_Empty__c2_ConnectionClosed
    lambda 14 In_SubscribeC2 = Out_Empty__c2_ConnectionClosed
    lambda 14 In_UnSubScribeC2 = Out_Empty__c2_ConnectionClosed
    lambda 14 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 14 In_DisconnectC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 16 In_ConnectC2 = Out_Empty__c2_ConnAck
    lambda 16 In_ConnectC1WithWill = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 16 In_ConnectC1WithWillRetain = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 16 In_DeleteRetainedC1 = Out_C1_PubAck__c2_ConnectionClosed
    lambda 16 In_DeleteRetainedC2 = Out_Empty__c2_ConnectionClosed
    lambda 16 In_SubscribeC2 = Out_Empty__c2_ConnectionClosed
    lambda 16 In_UnSubScribeC2 = Out_Empty__c2_ConnectionClosed
    lambda 16 In_DisconnectTCPC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda 16 In_DisconnectC1 = Out_C1_ConnectionClosed__c2_ConnectionClosed
    lambda _ _ = error "haal-gen: undefined transition"
