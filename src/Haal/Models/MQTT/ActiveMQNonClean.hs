...
module Haal.Models.MQTT.ActiveMQNonClean
    ( ActiveMQNonCleanInput (..)
    , ActiveMQNonCleanOutput (..)
    , activeMQNonClean
    ) where
...
activeMQNonClean :: MealyAutomaton Int ActiveMQNonCleanInput ActiveMQNonCleanOutput
activeMQNonClean = mkMealyAutomaton delta lambda (Set.fromList [0..11]) 0
  where
